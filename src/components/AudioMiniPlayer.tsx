...
export default function AudioMiniPlayer({ src, title }: AudioMiniPlayerProps) {
  ...
  return (
    <div className="flex items-center gap-3 pixel-border bg-[var(--c-surface)] px-4 py-2.5">
      ...
      <button
        onClick={togglePlay}
        className="w-8 h-8 flex items-center justify-center pixel-border-sm bg-[var(--c-primary)] hover:bg-[var(--c-primary-hover)] text-white text-sm transition-pixel shrink-0"
      >
        {isPlaying ? '||' : '\u25B6'}
      </button>
      ...
      <button
        onClick={cycleSpeed}
        className="font-[family-name:var(--font-press-start)] text-[8px] text-[var(--c-muted)] hover:text-[var(--c-fg)] pixel-border-sm bg-[var(--c-border)] px-2 py-1 transition-pixel shrink-0"
      >
        {playbackRate}x
      </button>
    </div>
